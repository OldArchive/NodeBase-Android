...
public class CoinMarketCapApiClient {
    ...
    public class NodeBaseMarket{
        ...
        public NodeBaseMarket(BigDecimal priceUsd, BigDecimal priceBtc, BigDecimal marketCapUsd, BigDecimal totalSupply, int rank) {
    ...
    public NodeBaseMarket getNodeBasePxrice() throws RequestNodeBaseRateException{
        try {
            NodeBaseMarket nodebaseMarket = null;
            String url = this.URL + "ndb";
            ...
            if (httpResponse.getStatusLine().getStatusCode()==200){
                ...
                String prieusd = jsonObject.getString("price_usd");
                double pusd = Double.parseDouble(prieusd) * 100;
                String prieubtc = jsonObject.getString("price_btc");
                double pbtc = Double.parseDouble(prieubtc) * 100;
                nodebaseMarket = new NodeBaseMarket(
                        new BigDecimal(pusd),
                        new BigDecimal(pbtc),
            ...
            return nodebaseMarket;
        } catch (ClientProtocolException e) {
            e.printStackTrace();
            throw new RequestNodeBaseRateException(e);
        } catch (IOException e) {
            e.printStackTrace();
            throw new RequestNodeBaseRateException(e);
    ...
    public static class BitPayApi {
         ...
         * @return
         * @throws RequestNodeBaseRateException
         */
        public <T> List<T> getRates(RatesConvertor<T> ratesConvertor) throws RequestNodeBaseRateException{
            try {
                HttpResponse httpResponse = get(URL);
                // receive response as inputStream
                InputStream inputStream = httpResponse.getEntity().getContent();
