package nodebase.org.nodebasewallet.rate.db;
...
import nodebase.org.nodebasewallet.contacts.AbstractSqliteDb;
...
public class RateDb extends AbstractSqliteDb<NodeBaseRate> implements RateDbDao<NodeBaseRate> {
    ...
    @Override
    protected ContentValues buildContent(NodeBaseRate obj) {
    ...
    @Override
    protected NodeBaseRate buildFrom(Cursor cursor) {
        ...
        return new NodeBaseRate(coin,value,timestap);
    }
    ...
    public void insertOrUpdateIfExist(NodeBaseRate nodebaseRate) {
        if (getRate(nodebaseRate.getCode())==null){
            insert(nodebaseRate);
        }else {
            updateByKey(KEY_COIN,nodebaseRate.getCode(),nodebaseRate);
        }
    }
}
