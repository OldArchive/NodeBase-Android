package nodebase.org.nodebasewallet.service;
...
import org.nodebasej.core.FilteredBlock;
import org.nodebasej.core.Peer;
import org.nodebasej.core.Transaction;
import org.nodebasej.core.TransactionConfidence;
import org.nodebasej.core.listeners.AbstractPeerDataEventListener;
import org.nodebasej.core.listeners.PeerConnectedEventListener;
import org.nodebasej.core.listeners.PeerDataEventListener;
import org.nodebasej.core.listeners.PeerDisconnectedEventListener;
import org.nodebasej.core.listeners.TransactionConfidenceEventListener;
import org.nodebasej.wallet.Wallet;
import org.nodebasej.wallet.listeners.WalletCoinsReceivedEventListener;
...
import nodebase.org.nodebasewallet.module.store.SnappyBlockchainStore;
import nodebase.org.nodebasewallet.rate.CoinMarketCapApiClient;
import nodebase.org.nodebasewallet.rate.RequestNodeBaseRateException;
...
import static nodebase.org.nodebasewallet.service.IntentsConstants.ACTION_ADDRESS_BALANCE_CHANGE;
import static nodebase.org.nodebasewallet.service.IntentsConstants.ACTION_BROADCAST_TRANSACTION;
import static nodebase.org.nodebasewallet.service.IntentsConstants.ACTION_CANCEL_COINS_RECEIVED;
...
import static nodebase.org.nodebasewallet.service.IntentsConstants.ACTION_SCHEDULE_SERVICE;
...
import static nodebase.org.nodebasewallet.service.IntentsConstants.NOT_BLOCKCHAIN_ALERT;
import static nodebase.org.nodebasewallet.service.IntentsConstants.NOT_COINS_RECEIVED;

/**
 * Created by akshaynexus on 6/12/17.
 */

public class NodeBaseWalletService extends Service{

    private Logger log = LoggerFactory.getLogger(NodeBaseWalletService.class);

    private NodeBaseApplication nodebaseApplication;
    private NodeBaseModuleImp module;
...
//    NotificationManager mNotificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    public class NodeBaseBinder extends Binder {
        public NodeBaseWalletService getService() {
            return NodeBaseWalletService.this;
        }
    }
        ...
        public void onBlocksDownloaded(final Peer peer, final Block block, final FilteredBlock filteredBlock, final int blocksLeft) {
            ...
            /*if (NodeBaseContext.IS_TEST)
                ...
                if (now - lastMessageTime > TimeUnit.SECONDS.toMillis(6)) {
                    ...
                    nodebaseApplication.getAppConf().setLastBestChainBlockTime(block.getTime().getTime());
    ...
    private class RunnableBlockChecker implements Runnable{
        ...
        @Override
        public void run() {
            org.nodebasej.core.Context.propagate(NodeBaseContext.CONTEXT);
    ...
    private WalletCoinsReceivedEventListener coinReceiverListener = new WalletCoinsReceivedEventListener() {
        ...
        public void onCoinsReceived(Wallet wallet, Transaction transaction, Coin coin, Coin coin1) {
            ...
            org.nodebasej.core.Context.propagate(CONTEXT);
            ...
            try {
                ...
                if (amount.isGreaterThan(Coin.ZERO)) {
                    ...
                    Intent resultIntent = new Intent(getApplicationContext(), NodeBaseWalletService.this.getClass());
                    resultIntent.setAction(ACTION_CANCEL_COINS_RECEIVED);
                    deleteIntent = PendingIntent.getService(NodeBaseWalletService.this, 0, resultIntent, PendingIntent.FLAG_CANCEL_CURRENT);
                        ...
                        mBuilder = new NotificationCompat.Builder(getApplicationContext())
                            ...
                            .setColor(
                                    (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ?
                                            ...
                                            :
                                            ContextCompat.getColor(NodeBaseWalletService.this, R.color.bgPurple))
    ...
    public void onCreate() {
        ...
        super.onCreate();
        try {
            log.info("NodeBase service started");
            // Android stuff
            ...
            module = (NodeBaseModuleImp) nodebaseApplication.getModule();
            blockchainManager = module.getBlockchainManager();
            // connect to pivtrum node
            /*pivtrumPeergroup = new PivtrumPeergroup(nodebaseApplication.getNetworkConf());
            ...
            String filename = NodeBaseContext.Files.BLOCKCHAIN_FILENAME;
            boolean fileExists = new File(file,filename).exists();
            blockchainStore = new SnappyBlockchainStore(NodeBaseContext.CONTEXT,file,filename);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        log.info("NodeBase service onStartCommand");
        try {
            if (intent != null) {
                try {
    ...
    private void requestRateCoin(){
        final AppConf appConf = nodebaseApplication.getAppConf();
        NodeBaseRate nodebaseRate = module.getRate(appConf.getSelectedRateCoin());
        if (nodebaseRate == null || nodebaseRate.getTimestamp() + NodeBaseContext.RATE_UPDATE_TIME < System.currentTimeMillis()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    try {
                        CoinMarketCapApiClient c = new CoinMarketCapApiClient();
                        CoinMarketCapApiClient.NodeBaseMarket nodebaseMarket = c.getNodeBasePxrice();
                        NodeBaseRate nodebaseRate = new NodeBaseRate("USD",nodebaseMarket.priceUsd,System.currentTimeMillis());
                        module.saveRate(nodebaseRate);
                        final NodeBaseRate nodebaseBtcRate = new NodeBaseRate("BTC",nodebaseMarket.priceBtc,System.currentTimeMillis());
                        module.saveRate(nodebaseBtcRate);

                        // Get the rest of the rates:
                        List<NodeBaseRate> rates = new CoinMarketCapApiClient.BitPayApi().getRates(new CoinMarketCapApiClient.BitPayApi.RatesConvertor<NodeBaseRate>() {
                            @Override
                            public NodeBaseRate convertRate(String code, String name, BigDecimal bitcoinRate) {
                                BigDecimal rate = bitcoinRate.multiply(nodebaseBtcRate.getRate());
                                return new NodeBaseRate(code,rate,System.currentTimeMillis());
                            }
                        });

                        for (NodeBaseRate rate : rates) {
                            module.saveRate(rate);
                        }

                    } catch (RequestNodeBaseRateException e) {
                        e.printStackTrace();
                    } catch (Exception e){
                        e.printStackTrace();
                    }
