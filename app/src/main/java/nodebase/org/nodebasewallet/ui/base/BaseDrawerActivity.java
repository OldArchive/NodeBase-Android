...
import nodebase.tech.akshaynexus.MasternodeListView;

import static nodebase.org.nodebasewallet.module.NodeBaseContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends NodeBaseActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = nodebaseApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = nodebaseModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (nodebaseModule.getChainHeight()+10>peerHeight) {
            ...
            if (nodebaseModule.isAnyPeerConnected()) {
                long peerHeight = nodebaseModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (nodebaseModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
    ...
    protected double calculateBlockchainSyncProgress() {
        long nodeHeight = nodebaseModule.getConnectedPeerHeight();
        if (nodeHeight>0){
            // calculate the progress
            // nodeHeight -> 100 %
            // current height -> x %
            return (nodebaseModule.getChainHeight()*100) / nodeHeight;
        }
        return -1;
    }
