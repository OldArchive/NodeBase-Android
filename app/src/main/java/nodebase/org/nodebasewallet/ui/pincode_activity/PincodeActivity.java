package nodebase.org.nodebasewallet.ui.pincode_activity;
...
import nodebase.org.nodebasewallet.ui.backup_mnemonic_activity.MnemonicActivity;
import nodebase.org.nodebasewallet.ui.base.BaseActivity;
import nodebase.org.nodebasewallet.ui.settings_pincode_activity.KeyboardFragment;
import nodebase.org.nodebasewallet.ui.start_activity.StartActivity;

import static nodebase.org.nodebasewallet.ui.backup_mnemonic_activity.MnemonicActivity.INTENT_EXTRA_INIT_VIEW;
...
public class PincodeActivity extends BaseActivity implements KeyboardFragment.onKeyListener {
    ...
    protected void onCreateView(Bundle savedInstanceState, ViewGroup container) {
        ...
        if (nodebaseApplication.getAppConf().getPincode()!=null && !checkPin){
            goNext();
            finish();
        }
    ...
    private void goNext() {
        if (nodebaseApplication.getAppConf().getTrustedNode()==null){
            // select random trusted node
            List<PivtrumPeerData> nodes = PivtrumGlobalData.listTrustedHosts();
            Random random = new Random();
            nodebaseApplication.setTrustedServer(nodes.get(random.nextInt(nodes.size())));
        ...
        if (lastPos<4) {
            if (key.getValue() < 10) {
                ...
                if (lastPos == 4) {
                    ...
                    if (!checkPin) {
                        nodebaseApplication.getAppConf().savePincode(pincode);
                        Toast.makeText(this, R.string.pincode_saved, Toast.LENGTH_SHORT).show();
                        goNext();
                    }else {
                        // check pin and return result
                        if(nodebaseApplication.getAppConf().getPincode().equals(pincode)){
