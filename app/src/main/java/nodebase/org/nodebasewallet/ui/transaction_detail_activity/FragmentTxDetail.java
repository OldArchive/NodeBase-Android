...
import org.nodebasej.core.TransactionOutPoint;
import org.nodebasej.core.TransactionOutput;
import org.nodebasej.script.Script;
...
import nodebase.org.nodebasewallet.ui.base.BaseFragment;
...
import static nodebase.org.nodebasewallet.ui.transaction_send_activity.custom.inputs.InputsActivity.INTENT_NO_TOTAL_AMOUNT;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (intent!=null){
            ...
            if (intent.hasExtra(IS_DETAIL)){
                transactionWrapper.setTransaction(nodebaseModule.getTx(transactionWrapper.getTxId()));
                isTxDetail = true;
            }else {
                transactionWrapper.setTransaction(new Transaction(nodebaseModule.getConf().getNetworkParams(),intent.getByteArrayExtra(TX)));
        ...
        if (transactionWrapper.isStake()){
            ...
            try {
                ...
                for (TransactionInput input : transactionWrapper.getTransaction().getInputs()) {
                    ...
                    inputsSum = inputsSum.plus(nodebaseModule.getUnspentValue(unspent.getHash(), (int) unspent.getIndex()));
                }
            ...
            if (transactionWrapper.getOutputLabels()!=null && transactionWrapper.getOutputLabels().containsKey(transactionOutput.getIndex())){
                ...
                if (addressLabel !=null) {
                    if (addressLabel.getName() != null) {
                        ...
                        label = transactionOutput.getScriptPubKey().getToAddress(nodebaseModule.getConf().getNetworkParams(),true).toBase58();
                }else {
                    label = transactionOutput.getScriptPubKey().getToAddress(nodebaseModule.getConf().getNetworkParams(),true).toBase58();
                }
            }else {
                ...
                if (script.isPayToScriptHash() || script.isSentToRawPubKey() || script.isSentToAddress()) {
                    label = script.getToAddress(nodebaseModule.getConf().getNetworkParams(), true).toBase58();
                }else {
    ...
    private void setupOutputs(List<OutputUtil> list) {
        ...
        ListItemListeners<OutputUtil> listItemListener = new ListItemListeners<OutputUtil>() {
            ...
            public void onLongItemClickListener(OutputUtil data, int position) {
                if (nodebaseModule.chechAddress(data.getLabel())) {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.txt_inputs){
            try {
                ...
                bundle.putSerializable(INTENT_EXTRA_UNSPENT_WRAPPERS, (Serializable) nodebaseModule.convertFrom(transactionWrapper.getTransaction().getInputs()));
