...
import nodebase.org.nodebasewallet.ui.transaction_detail_activity.TransactionDetailActivity;
import global.wrappers.TransactionWrapper;

import static nodebase.org.nodebasewallet.ui.transaction_detail_activity.FragmentTxDetail.IS_DETAIL;
import static nodebase.org.nodebasewallet.ui.transaction_detail_activity.FragmentTxDetail.TX_WRAPPER;
import static nodebase.org.nodebasewallet.utils.TxUtils.getAddressOrContact;
...
public class TransactionsFragmentBase extends BaseRecyclerFragment<TransactionWrapper> {
    ...
    protected List<TransactionWrapper> onLoading() {
        List<TransactionWrapper> list = nodebaseModule.listTx();
        ...
        BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase> adapter = new BaseRecyclerAdapter<TransactionWrapper, TransactionViewHolderBase>(getActivity()) {
            ...
            protected void bindHolder(TransactionViewHolderBase holder, TransactionWrapper data, int position) {
                ...
                if (nodebaseRate!=null) {
                    localCurrency = nodebaseApplication.getCentralFormats().format(
                                    new BigDecimal(data.getAmount().getValue() * nodebaseRate.getRate().doubleValue()).movePointLeft(8)
                                    )
                ...
                holder.title.setText(getAddressOrContact(nodebaseModule,data));
                ...
                /*if (data.getOutputLabels()!=null && !data.getOutputLabels().isEmpty()){
                    ...
                    if (contact!=null) {
                        ...
                        holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(nodebaseModule.getConf().getNetworkParams()).toBase58());
                    }
                }else {
                    holder.title.setText(data.getTransaction().getOutput(0).getScriptPubKey().getToAddress(nodebaseModule.getConf().getNetworkParams()).toBase58());
                }*/
